...
(async () => {
  ...
  const psgBorders = await p11.$$eval('.psg, .psg-text', els => els.map(e => getComputedStyle(e).borderTopWidth));
  ok(psgBorders.every(w => w === '0px'), '지문 상자 테두리 없음');
  await p11.close();
